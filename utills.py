...
def load_data(dataset_path, mode, batch_size):
    dataset = VCTKDataset(
        dataset_path, mode, sample_size=sample_size, train_ratio=train_ratio
    )
...
def compute_mse(spectrogram1, spectrogram2):
    ...
    assert (
        spectrogram1.shape == spectrogram2.shape
    ), "Spectrograms must have the same shape"
...
def save_samples(predictions, inputwave):
    ...
    for i, pred in enumerate(predictions):
        ...
        waveform_reg = invert_stft_to_audio(
            pred, inputwave[i].detach().numpy(), hop_length=512
        )
